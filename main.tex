...
\newcommand{\correspondingauthor}[1]{Stefan Hartmann, University of Düsseldorf, hartmast@hhu.de}
...
\vspace{4pt}
\large
    Stefan Hartmann\textsuperscript{1}, Random Author\textsuperscript{2} \& Another Random Author\textsuperscript{2}
    
\small
   \textsuperscript{1} University of Düsseldorf \textsuperscript{2} Random University 
...
\begin{table}[H]
\sffamily \begin{tabularx}{\textwidth} { 
  | L | L | R | }
 \hline
 \rowcolor[gray]{.5}\multicolumn{3}{|>{\hsize=\dimexpr3\hsize+4\tabcolsep+2\centering\relax}X|}{\textcolor{white}{Complicated example table}} \\
 \hline
 \rowcolor[gray]{.9} \textbf{Lorem ipsum} & \textbf{Example} & \textbf{Frequency} \\
 \hline
 Lorem  & Lorem ipsum dolor sit amet  &  42  \\
 \hline
 ipsum & bla bla blubb & 21 \\
\hline
\multicolumn{2}{|>{\hsize=\dimexpr2\hsize+2\tabcolsep+\raggedright\relax}X|}{\textbf{Sum}} & \textbf{63} \\
 \hline
  \end{tabularx}
...
\begin{itemize}
    \item Use \verb!\citep{...}! for inserting citations in parantheses, e.g. \citep{Goldberg2003}.
    \item Use \verb!\citet{...}! to insert citations in the format Author/s (Year), as in \citet{Goldberg1995}.
    \item Use \verb!\citeauthor{...}! and \verb!\citeyear{...}! if you just want to cite the author or the year, without parantheses. This can be helpful if you cite something within parantheses (as in this example, see \citeauthor{croftRadicalConstructionGrammar2001} \citeyear{croftRadicalConstructionGrammar2001}).
    \item If you want to use a possessive 's in references, you can use the custom command \verb!\citegen{...}! defined in the localstuff.tex file, as in: \citegen{Goldberg1995} definition of constructions.
\end{itemize}
...
\begin{itemize}
    \item Some Zotero plugins (like Better BibTeX) change the ``year" field in the BibTeX entry to ``date". In this case, the year will unfortunately not be displayed.
    \item If you want to keep sentence-internal capitals in the citations (e.g. when quoting German titles, such as \citeauthor{hartmannSchemaUndInstanz2021} \citeyear{hartmannSchemaUndInstanz2021}), make sure that the relevant words are enclosed in curly brackets. If you export the BibTeX entries from Zotero, you usually won't have to worry because it does so automatically.
\end{itemize}
